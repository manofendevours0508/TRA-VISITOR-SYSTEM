...
export default function DocumentSearch() {
  ...
  return (
    <StaffLayout>
      <h1 className="text-2xl font-bold text-tra-black mb-6">{t('documentSearchTitle')}</h1>

      <form onSubmit={handleSearch} className="flex gap-3 mb-6">
        <input
          value={q}
          onChange={(e) => setQ(e.target.value)}
          placeholder={t('searchByReferenceEtc')}
        ...
        <div className="bg-white rounded-xl shadow overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {results.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-6 text-center text-slate-400">{t('noDocumentsMatched')}</td></tr>
              )}
            </tbody>
          </table>
        </div>
